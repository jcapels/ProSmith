...
def evaluate(gpu, args, device):
    for seed in range(5):
        ...
        if os.path.exists(join(args.save_pred_path, "data.npz")):
            # Load all variables from the .npz file
            data = np.load(join(args.save_pred_path, 'data.npz'))
            ...
            np.savez(
                join(args.save_pred_path, 'data.npz'),
        ...
        if os.path.exists(join(args.save_pred_path, "results.csv")):
            file_exists=True
        else:
            file_exists=False

        df.to_csv(join(args.save_pred_path, "results.csv"), mode='a', index=False, header=not file_exists)
